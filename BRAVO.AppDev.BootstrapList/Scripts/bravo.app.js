...
BRAVO.App = function () {
    ...
    var contentType_AddFields = function (web, parent, ct, ctInfo) {
        ...
        var flTitle = contentType.get_fieldLinks().getById("fa564e0f-0c70-4ab9-b863-0177e6ddd247");

        // Load the objects
        context.load(src);
        context.load(contentType);
        context.load(flTitle);
        ...
        for (var i = 0; i < ctInfo.FieldRefs.length; i++) {
            ...
            if (ctInfo.FieldRefs[i] == "Title") {
                // Note - The 'Title' field is defaulted to "Hidden" when clearing the content type field links
                // Ensure this field is visible
                flTitle.set_hidden(false);
            }
            else {
                // Add the field link
        ...
        contentType.get_fieldLinks().reorder(ctInfo.FieldRefs);
        ...
        file_Get(web, fileUrl).done(function (file) {
            ...
            if (file.exists) {
                ...
                if (removeFl) {
                    ...
                    // Ensure the file is currently checked in
                    if (file.CheckOutType != SP.CheckOutType.none) {
                        // Add a validation message
                        _validationResults.push({ Category: logCategory, Type: BRAVO.App.ValidationType.Warning, Message: "The file '" + fileName + "' is not checked in." });
                    }
    ...
    var file_UpdateProperties = function (file, fileInfo) {
        ...
        file.get_ListItemAllFields().done(function (item) {
            ...
            if (item.exists) {
                ...
                // Update the item
                item.update(properties).done(function (result) {
    ...
    var list_UpdateView = function (list, viewInfo) {
        ...
        list.getViewByTitle(viewInfo.Data.Title).done(function (view) {
            ...
            if (view.exists && viewInfo.ViewFields && viewInfo.ViewFields.length > 0) {
                // Get the view fields
